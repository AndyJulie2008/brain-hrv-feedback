...
music_list = ["Hans Zimmer_time.mp3", "Relaxing_Ocean_Wave.mp3", "Interstellar.mp3","Destroyer_Of_Worlds.mp3","White_Noise_1min.mp3","Bandari_Silence_With_Sound_From_Nature.mp3", "Night_OCEAN_WAVES_3hrs.mp3"]
bg_music = pygame.mixer.Sound(os.path.join(music_dir,music_list[3])) # Reward for arousal
...
class MuseDashboard(QtWidgets.QMainWindow):
    def __init__(self,
        ...
        # === Biofeedback circles ===
        # Meditation circle
        self.circle_plot = self.plotWidget.addPlot(row=1, col=0)
        ...
        self.meditation_title.setText("Meditation: r ∝ 1/(θ/α) ∝ (α/θ)", color='w', size='30pt')
        self.plotWidget.addItem(self.meditation_title, row=2, col=0)

        # Arousal circle
        self.circle_arousal_plot = self.plotWidget.addPlot(row=1, col=1)
        ...
        self.arousal_title.setText("Arousal: r ∝ 1/(θ/β) ∝ (β/θ)", color='w', size='30pt')
        self.plotWidget.addItem(self.arousal_title, row=2, col=1)

        self.labels = {}
        for key in ['Meditation', 'Arousal']:
        ...
        with lock:
            ...
            if time.time() - self.sampling_start_time < self.sampling_duration:
                ...
                if time.time() - self.sampling_start_time < self.burn_in_period:
                    ...
                    if recording and csv_writer:
                        row = [time.time(), concentration, mellow] + \
                            ...
                            gyro_data + acc_data + [int(blink_csv), int(jaw_csv), 'Burn-in']
                ...
                if recording and csv_writer:
                    row = [time.time(), concentration, mellow] + \
                        ...
                        [beta_buffers[ch][-1] if beta_buffers[ch] else 0.0 for ch in channels] + \
                        [gamma_buffers[ch][-1] if gamma_buffers[ch] else 0.0 for ch in channels] + \
                        gyro_data + acc_data + [int(blink_csv), int(jaw_csv), 'Calibration']
                    csv_writer.writerow(row)
            ...
            if recording and csv_writer:
                row = [time.time(), concentration, mellow] + \
                        ...
                        [beta_buffers[ch][-1] if beta_buffers[ch] else 0.0 for ch in channels] + \
                        [gamma_buffers[ch][-1] if gamma_buffers[ch] else 0.0 for ch in channels] + \
                        gyro_data + acc_data + [int(blink_csv), int(jaw_csv), 'Test']
                csv_writer.writerow(row)

    def toggle_recording(self):
        global recording, csv_writer, csv_file
        if not recording:
            filename = f"muse_recording_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            ...
            headers = ['Time', 'Concentration', 'Mellow'] + \
                      ...
                      ['Gyro_X', 'Gyro_Y', 'Gyro_Z', 'Acc_X', 'Acc_Y', 'Acc_Z', 'Blink', 'Jaw', 'Status']
...
def run_app_gui():
    ...
    # Pop out the window to input parameters
